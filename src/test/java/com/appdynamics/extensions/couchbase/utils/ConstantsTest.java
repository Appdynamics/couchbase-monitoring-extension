...
import com.appdynamics.extensions.couchbase.utils.Constants;
...
public class ConstantsTest {
    ...
    public void getMetricsTest() throws IOException{
        ...
        JsonNode metricsNode = mapper.readValue(new File("src/test/resources/MetricsNode.json"), JsonNode.class);
        List<Metric> resultMetricList = getMetrics("Custom metrics|Redis|total", metricsList, metricsNode);
    ...
    public void getMetricsFromArrayTest() throws IOException{
        ...
        metricsList.add(metricsMap);
        ObjectMapper mapper = new ObjectMapper();
        JsonNode metricsNode = mapper.readValue(new File("src/test/resources/MetricNodeWithArray.json"), JsonNode.class);
        List<Metric> resultMetricList = getMetricsFromArray("Custom metrics|Redis|timestamp", metricsList, metricsNode);
        Assert.assertTrue(resultMetricList.size() == 1);
